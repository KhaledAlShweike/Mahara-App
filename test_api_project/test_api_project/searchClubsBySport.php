...
if (empty($_GET['q']) || empty($_GET['location_id']) || empty($_GET['sport_id'])) {
    echo json_encode(array("status" => "-1", "error" => "enter query"));
} else {
    $query = "%" . $_GET['q'] . "%";
    $locationID = $_GET['location_id'];
    $sportID = $_GET['sport_id'];
    $stm = $connect->prepare("SELECT clubs.id,clubs.name,clubs.address from clubs,clubsports where clubs.location_id = ? AND clubsports.club_id = clubs.id AND clubsports.sport_type_id = ? AND name like ? order by  clubs.name");
    $stm->execute(array($locationID,$sportID,$query));
