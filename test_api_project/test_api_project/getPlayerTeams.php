...
if(empty($_GET['player_id'])){
    ...
    $stm = $connect->prepare("SELECT teams.id,teams.name,sporttypes.SportType FROM `teams`,teamplayers,sporttypes WHERE teams.sport_type_id = sporttypes.id AND teamplayers.team_id = teams.id and teamplayers.player_id = ?");
